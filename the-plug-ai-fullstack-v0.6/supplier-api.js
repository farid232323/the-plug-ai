...
module.exports=function createSupplierApi({db,json,readJson,adminAuthorized,adminChallenge}){
  ...
  function productRows(items){
    const ids=[...new Set((items||[]).map(x=>Number(x.product_id||x.id)).filter(Number.isFinite))];
    if(!ids.length)return [];
    const qs=ids.map(()=>'?').join(',');
    return db.prepare(`SELECT id,brand_name,mfg_part_id,title,price_sar,status FROM products WHERE id IN (${qs})`).all(...ids);
  }
  ...
  async function quote(req,res){
    ...
    if(rows.length!==new Set(items.map(x=>Number(x.product_id||x.id)).filter(Number.isFinite)).size)return json(res,{error:'One or more products could not be matched to the catalog.'},400);
