(()=>{
  ...
  let DATA={brands:{}},brand='',model='',submodel='',spec=null,family='';
  ...
  style.textContent=`
    ...
    .tp3-family-tabs{display:flex;flex-wrap:wrap;gap:6px;padding:10px 14px;border-bottom:1px solid #f0f2f4}.tp3-family-tab{border:1px solid #dfe5ea;background:#fff;color:#132135;font:700 11px/1 Arial,sans-serif;text-transform:uppercase;letter-spacing:.02em;padding:7px 11px;border-radius:999px;cursor:pointer}.tp3-family-tab.active{background:#132135;border-color:#132135;color:#fff}
  ...
  function familyOf(name){const n=String(name||'').trim();if(/series/i.test(n))return n;const m=n.match(/^([A-Za-z]+)\d/);if(m)return m[1]+' Series';if(brand==='BMW'){const d=n.match(/^(\d)/);if(d)return d[1]+' Series'}return n}
  const families=()=>[...new Set(models().map(familyOf))].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
  ...
  function render(step='brand'){
    const p=shell(),bs=brands();if(!brand||!DATA.brands[brand])brand=bs[0]||'';
    const ms=models();const fs=families();if(!family||!fs.includes(family))family=fs[0]||'';
    const q=$('.tp3-search input',p)?.value.trim().toLowerCase()||'';
    const fms=q?ms:ms.filter(x=>familyOf(x)===family);
    if(!model||!fms.includes(model))model=fms[0]||'';
    const ss=subs();if(!submodel||!DATA.brands?.[brand]?.[model]?.[submodel])submodel=ss[0]||'';const sp=specs();if(spec&&!sp.includes(spec))spec=null;
    ...
    $('.tp3-model',p).innerHTML=`<div class="tp3-label"><button class="tp3-mobile-back" data-back="brand">←</button><b>2</b>Select Model</div><div class="tp3-family-tabs">${fs.map(f=>`<button class="tp3-family-tab ${f===family?'active':''}" data-f="${esc(f)}">${esc(f)}</button>`).join('')}</div>${filtered(fms).map(x=>`<button class="tp3-choice ${x===model?'active':''}" data-m="${esc(x)}">${esc(x)}<span>›</span></button>`).join('')||'<div class="tp3-empty">Choose a brand first.</div>'}`;
