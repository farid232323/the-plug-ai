...
module.exports=function createShopifyAdapter({headless=true,dataDir='/data'}={}){
  ...
  // Diagnostic-only: records which host a P3 lookup step hit and what came back, without ever
  // logging the request path/query (which can carry SKU/search text) or any headers/cookies/tokens.
  function logLookup(s,step,base,r,err){
    let hostname='',finalUrl='';
    try{hostname=new URL(base).hostname}catch{}
    try{if(r){const fu=new URL(r.url());fu.search='';fu.hash='';finalUrl=fu.toString()}}catch{}
    console.log('[supplier-lookup]',JSON.stringify({slug:s?.slug||'',step,hostname,status:r?r.status():null,content_type:r?(r.headers()['content-type']||null):null,final_url:finalUrl||null,error:err?String(err.message||err):null}));
  }
  ...
  async function shopifyJsonLookup(page,base,item,s){
    ...
    for(const term of candidates){
      ...
      }catch(err){logLookup(s,'predictive_search',base,null,err)}
    }
    ...
    }catch(err){logLookup(s,'products_json',base,null,err)}
    return '';
  }
  async function p3CollectionLookup(page,base,item,s){
  ...
  async function p3CollectionHtmlLookup(page,base,item,s){
    const sku=norm(item.sku||item.mfg_part_id);if(!sku)return '';
    try{
      for(let pageNo=1;pageNo<=4;pageNo++){
        const resp=await page.goto(`${base}/collections/all?page=${pageNo}`,{waitUntil:'domcontentloaded',timeout:45000});
        logLookup(s,'collection_html_page',base,resp);
        ...
        for(const href of hrefs){
          try{
            ...
            const r=await page.request.get(productUrl.toString()+'.js',{timeout:12000,headers:{accept:'application/json'}});
            logLookup(s,'collection_html_product',base,r);
            if(!r.ok())continue;
            const p=await r.json().catch(()=>null);if((p?.variants||[]).some(v=>norm(v?.sku)===sku))return productUrl.toString();
          }catch(err){logLookup(s,'collection_html_product',base,null,err)}
        }
      }
    }catch(err){logLookup(s,'collection_html_crawl',base,null,err)}
    return '';
  }
  async function htmlSearchLookup(page,base,item,s){
    const candidates=productCandidates(item);
    for(const term of candidates){
      try{
        const resp=await page.goto(`${base}/search?q=${encodeURIComponent(term)}`,{waitUntil:'domcontentloaded',timeout:45000});
        logLookup(s,'html_search',base,resp);
      ...
      }catch(err){logLookup(s,'html_search',base,null,err)}
    }
    return '';
  }
  ...
  async function addP3(page,s,item,url,qty){
    const sku=norm(item.sku||item.mfg_part_id);if(!sku)throw new Error('P3 SKU is missing');
    const r=await page.request.get(url.replace(/\/$/,'')+'.js',{timeout:12000,headers:{accept:'application/json'}});if(!r.ok())throw new Error(`P3 product data request failed (${r.status()})`);
    const p=await r.json().catch(()=>null),variant=(p?.variants||[]).find(v=>norm(v?.sku)===sku);if(!variant?.id)throw new Error(`Unable to resolve P3 variant for ${clean(item.sku||item.mfg_part_id)}`);
    const base=s.base_url.replace(/\/$/,'');const ar=await page.request.post(`${base}/cart/add.js`,{timeout:15000,headers:{accept:'application/json'},form:{id:String(variant.id),quantity:String(Math.max(1,Number(qty||1)))}});if(!ar.ok())throw new Error(`P3 cart add failed (${ar.status()})`);
  }
  ...
  async function quote(s,payload){
    ...
    const {browser,context}=await contextFor(s);try{const page=await context.newPage();await page.goto(`${s.base_url.replace(/\/$/,'')}/cart/clear`,{waitUntil:'domcontentloaded',timeout:45000}).catch(()=>{});const stocks=[];const isP3=s.slug==='p3-gauges'||s.slug==='p3-guages';for(const item of items){const url=await resolveProduct(page,s,item),st=await stock(page);stocks.push({product_id:item.product_id||null,sku:item.sku||'',url,...st});if(!st.available)return{ok:true,supplier:s.name,available:false,stocks,shipping:null};if(isP3)await addP3(page,s,item,url,item.qty);else await add(page,Math.max(1,Number(item.qty||1)))}const all=await rates(page,s,payload.address||{});await save(s,context);if(!all.length)return{ok:false,supplier:s.name,available:true,stocks,error:'No Shopify shipping rate was returned for this address'};const best=all[0];return{ok:true,supplier:s.name,available:true,stocks,shipping:{amount:best.price,source_currency:best.currency,method:best.name,all_rates:all,quoted_at:new Date().toISOString()},rate_source:'shopify_cart_api'}}finally{await browser.close()}
  }
