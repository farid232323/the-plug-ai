...
module.exports=function createValvetronicAuth(opts={}){
  ...
  async function start(email){
    ...
    try{
      ...
      const awaiting=/code|verification|one-time|otp/i.test(s.body)||!!(await findField(page,'code'));
  ...
  async function verify(code){
    ...
    try{
      ...
      await settle(page);
      const codeField=await findField(page,'code');
      if(!codeField)throw new Error(`Valvetronic verification-code field was not found at ${page.url()}`);
      await codeField.loc.fill(otp);
      let submitted=false;
      for(const name of [/submit/i,/verify/i,/continue/i,/sign in/i]){
        const btn=codeField.frame.getByRole('button',{name}).first();
        if(await btn.count().catch(()=>0)){await btn.click({timeout:12000});submitted=true;break}
      }
      if(!submitted)await codeField.loc.press('Enter');
      await settle(page);
