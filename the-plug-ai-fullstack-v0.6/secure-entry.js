...
async function publicCarousel(res){try{const r=await internalRequest('/api/admin/settings');const settings=JSON.parse(r.body.toString('utf8')||'{}');let slides=defaultSlides;if(settings.carousel_slides){try{const parsed=JSON.parse(settings.carousel_slides);if(Array.isArray(parsed)&&parsed.length)slides=parsed}catch{}}slides=slides.filter(x=>x&&x.enabled!==false&&x.image).map(x=>({image:String(x.image),label:String(x.label||'European performance'),position:String(x.position||'center center')}));const body=Buffer.from(JSON.stringify({slides:slides.length?slides:defaultSlides}));res.writeHead(200,{'content-type':'application/json','content-length':body.length,'cache-control':'no-store'});res.end(body)}catch{const body=Buffer.from(JSON.stringify({slides:defaultSlides}));res.writeHead(200,{'content-type':'application/json','content-length':body.length});res.end(body)}}
async function appendScript(res,sourcePath,extraPath){try{const r=await internalRequest(sourcePath);const extra=fs.readFileSync(path.join(__dirname,'public',extraPath));const body=Buffer.concat([r.body,Buffer.from('\n;'),extra]);res.writeHead(r.status,{'content-type':'application/javascript; charset=utf-8','content-length':body.length,'cache-control':'no-store'});res.end(body)}catch{res.writeHead(500,{'content-type':'text/plain'});res.end('Unable to load scripts')}}
