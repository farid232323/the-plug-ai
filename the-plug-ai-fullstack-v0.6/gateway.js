...
function customerFromToken(token){if(!token)return null;const h=crypto.createHash('sha256').update(String(token)).digest('hex');return db.prepare(`SELECT c.id,c.first_name,c.last_name,c.email,c.phone,s.expires_at FROM customer_sessions s JOIN customers c ON c.id=s.customer_id WHERE s.token_hash=? AND datetime(s.expires_at)>datetime('now')`).get(h)||null}
function customerPayload(c,token){return {id:c.id,first_name:c.first_name||'',last_name:c.last_name||'',email:c.email||'',phone:c.phone||'',token}}
...
async function shopApi(req,res,u){const p=u.pathname,d=await readJson(req);
  if(p==='/api/shop/register'&&req.method==='POST'){
    const email=clean(d.email).toLowerCase(),password=String(d.password||'');if(!validEmail(email))return json(res,{error:'Enter a valid email address.'},400);if(password.length<8)return json(res,{error:'Password must be at least 8 characters.'},400);if(db.prepare('SELECT id FROM customers WHERE lower(email)=lower(?)').get(email))return json(res,{error:'An account already exists with this email.'},409);
    const r=db.prepare('INSERT INTO customers(first_name,last_name,email,phone,password_hash,status) VALUES(?,?,?,?,?,?)').run(clean(d.first_name),clean(d.last_name),email,clean(d.phone),hashPassword(password),'active');const c=db.prepare('SELECT id,first_name,last_name,email,phone FROM customers WHERE id=?').get(r.lastInsertRowid);const token=issueSession(c.id);
    const html=emailShell('Welcome to The Plug',`<p>Hi ${clean(c.first_name)||'there'},</p><p>Your account has been created successfully. You can now sign in, keep your orders together and check out faster.</p>`);await sendEmail('account_created',email,'Welcome to The Plug',html);notifyAdmin('account_created','New The Plug account created',emailShell('New customer account',`<p>${clean(c.first_name)} ${clean(c.last_name)}</p><p>${email}</p>`));
    return json(res,{customer:customerPayload(c,token)},201);
  }
  if(p==='/api/shop/login'&&req.method==='POST'){
    const email=clean(d.email).toLowerCase(),c=db.prepare('SELECT * FROM customers WHERE lower(email)=lower(?) AND status=?').get(email,'active');if(!c||!verifyPassword(d.password,c.password_hash))return json(res,{error:'Incorrect email or password.'},401);const token=issueSession(c.id);return json(res,{customer:customerPayload(c,token)});
  }
  ...
  if(p==='/api/shop/order'&&req.method==='POST'){
    ...
    const r=db.prepare('INSERT INTO orders(order_no,customer_name,email,phone,total,status,customer_id) VALUES(?,?,?,?,?,?,?)').run(no,clean(d.customer_name)||'Guest',email,clean(d.phone),total,'placed',user?.id||null);for(const x of items)db.prepare('INSERT INTO order_items(order_id,product_id,title,qty,price) VALUES(?,?,?,?,?)').run(r.lastInsertRowid,x.product_id||null,clean(x.title)||'Product',Number(x.qty||1),Number(x.price||0));
    const rows=items.map(x=>`<tr><td style="padding:8px 0">${clean(x.title)||'Product'} × ${Number(x.qty||1)}</td><td style="padding:8px 0;text-align:right">${money(Number(x.price||0)*Number(x.qty||1))}</td></tr>`).join('');const html=emailShell(`Order ${no} confirmed`,`<p>Thank you for your order.</p><table style="width:100%;border-collapse:collapse">${rows}<tr><td style="padding-top:14px;border-top:1px solid #ddd"><b>Total</b></td><td style="padding-top:14px;border-top:1px solid #ddd;text-align:right"><b>${money(total)}</b></td></tr></table><p>We will contact you with the next order update.</p>`);await sendEmail('order_created',email,`Order ${no} confirmed`,html);notifyAdmin('order_created',`New order ${no}`,emailShell('New order received',`<p><b>${no}</b></p><p>${clean(d.customer_name)} · ${email}</p><p>Total: <b>${money(total)}</b></p>`));
    return json(res,{order_no:no,id:r.lastInsertRowid,status:'placed'},201);
  }
  return false;
}

function proxyRequest(req,res,bodyOverride=null){const headers={...req.headers,host:`127.0.0.1:${APP_PORT}`};if(bodyOverride)headers['content-length']=bodyOverride.length;const pr=http.request({hostname:'127.0.0.1',port:APP_PORT,path:req.url,method:req.method,headers},pres=>{if(req.url.split('?')[0]==='/api-storefront.js'&&req.method==='GET'){const chunks=[];pres.on('data',c=>chunks.push(c));pres.on('end',()=>{try{const extra=fs.readFileSync(path.join(ROOT,'public','customer-commerce.js'));const body=Buffer.concat([Buffer.concat(chunks),Buffer.from('\n;'),extra]);res.writeHead(pres.statusCode||200,{...pres.headers,'content-type':'application/javascript; charset=utf-8','content-length':body.length,'cache-control':'no-store'});res.end(body)}catch(e){res.writeHead(500);res.end('Unable to load storefront')}});return}res.writeHead(pres.statusCode||500,pres.headers);pres.pipe(res)});pr.on('error',e=>{console.error('Gateway proxy error',e);if(!res.headersSent)res.writeHead(502);res.end('Bad gateway')});if(bodyOverride)pr.end(bodyOverride);else req.pipe(pr)}

const server=http.createServer(async(req,res)=>{const u=new URL(req.url,'http://localhost');if(u.pathname.startsWith('/api/shop/')){const handled=await shopApi(req,res,u);if(handled!==false)return}proxyRequest(req,res)});
