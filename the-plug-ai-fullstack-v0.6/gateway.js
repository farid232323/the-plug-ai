...
function proxyRequest(req,res,bodyOverride=null){const headers={...req.headers,host:`127.0.0.1:${APP_PORT}`};if(bodyOverride)headers['content-length']=bodyOverride.length;const pr=http.request({hostname:'127.0.0.1',port:APP_PORT,path:req.url,method:req.method,headers},pres=>{if(req.url.split('?')[0]==='/api-storefront.js'&&req.method==='GET'){const chunks=[];pres.on('data',c=>chunks.push(c));pres.on('end',()=>{try{const extras=['customer-commerce.js','customer-account-polish.js'].map(f=>fs.readFileSync(path.join(ROOT,'public',f)));const body=Buffer.concat([Buffer.concat(chunks),Buffer.from('\n;'),extras[0],Buffer.from('\n;'),extras[1]]);res.writeHead(pres.statusCode||200,{...pres.headers,'content-type':'application/javascript; charset=utf-8','content-length':body.length,'cache-control':'no-store'});res.end(body)}catch(e){console.error('Unable to load storefront commerce scripts',e);res.writeHead(500);res.end('Unable to load storefront')}});return}res.writeHead(pres.statusCode||500,pres.headers);pres.pipe(res)});pr.on('error',e=>{console.error('Gateway proxy error',e);if(!res.headersSent)res.writeHead(502);res.end('Bad gateway')});if(bodyOverride)pr.end(bodyOverride);else req.pipe(pr)}
